...
import pl.biometria.voice.recognition.distances.DistanceCalculator;
import pl.biometria.voice.recognition.distances.EuclideanDistanceCalculator;
import pl.biometria.voice.recognition.enhancements.Normalizer;
import pl.biometria.voice.recognition.features.FeaturesExtractor;
import pl.biometria.voice.recognition.features.LpcFeaturesExtractor;
import pl.biometria.voice.recognition.vad.AutocorrellatedVoiceActivityDetector;
import pl.biometria.voice.utils.FileHelper;

/**
 * Front class for accessing Recognito's speaker recognition features
 * <p>
 * {@code Recognito} holds a set of voice prints associated with user keys and allows execution of
 ...
 * <li>Speaker recognition : analyse voice characteristics from an unknown sample and return a
 * {@code List} of {@code MatchResult}s sorted by distance. A likelihood ratio is provided within
 * each {@code MatchResult}.</li>
 * </ul>
 * <p>
 * {@code Recognito} expects all voice samples to be comprised of a single channel (i.e. mono).
 * Using a stereo sample whose channels are identical will merely double processing time. Using a
 * real stereo sample will make the processing less accurate while doubling processing time.<br/>
 * </p>
 * <p>
 * It is up to the user to manage persistence of the created voice print objects. Persisted voice
 * prints may be passed into an alternate {@code Recognito} constructor as a {@code Map} of user
 * keys pointing to a voice print.
 * </p>
 * <p>
 * For methods taking a file handle :<br/>
 * Supporting each and every file formats is a real pain and not the primary goal of Recognito. As
 * such, the conversion capabilities of the javax.sound.sampled package are used internally.
 * Depending on your particular JVM implementation, some file types may or may not be supported. If
 * you're looking for MP3 or Ogg support, check Javazoom SPI's. This said, the higher the sample
 * quality, the better the results. In case you may choose, {@code Recognito}'s preferred file
 * format is PCM 16bit mono 16000 Hz (WAV files are PCM)<br/>
 * You may also want to check http://sox.sourceforge.net for dedicated conversion software.
 * </p>
 * <p>
 * Please note the sample rate is actually twice the highest audio frequency of the sample. E.g. a
 * sample rate of 8KHz means that the highest frequency available in the sample is 4KHz. So you
 * can't resample at a higher frequency and expect the voice samples to be comparable, some
 * frequencies will be missing. You may downsample, but if don't know how to do that correctly,
 * you're better off using dedicated software. For the purpose of extracting voice prints, 16KHz
 * appears to be the most interesting choice.
 * </p>
 * <p>
 * The likelihood ratio available within the {@code MatchResult}s is calculated as the relative
 * distance between the given voice sample, a known {@code VoicePrint} and a so called Universal
 * Model. The universal model in {@code Recognito} is by default created as an average of all
 * {@code VoicePrint}s available in the system. The closer you are to the known {@code VoicePrint},
 * the higher the likelihood. Each time a new sample is sent to {@code Recognito}'s create or merge
 * methods, the extracted features are added to the model. You may create your own model by merging
 * a selected set of voice samples into a single {@code VoicePrint}. Once done, you may set this
 * model once and for all in {@code Recognito}, it won't be updated afterwards. A Universal Model is
 * language dependent. At this very moment, it doesn't look realistic that {@code Recognito} would
 * provide generic models for each and every language. Furthermore, the recording system you're
 * using will also severely impact the relevance of a generic model in that the sonic
 * characteristics will be quite different from one system to another. In other words, providing
 * generic models would most probably create likelihood ratios that are unreasonably high because
 * far from your own recordings and thus irrelevant.
 * </p>
 * <p>
 * Threading : usage of {@code Recognito} is thread safe, see methods documentation for details
 * </p>
 *
 * @param <K> {@code Recognito} is genericized in order to allow the user to specify its own type of
 *        user keys. The constraints on user keys are the same as those for a {@code java.util.Map}
 *        key
...
public class Recognito<K> {
   ...
   *
   * @param sampleRate the sample rate, at least 8000.0 Hz (preferably higher)
   * @param voicePrintsByUserKey a {@code Map} containing user keys and their respective
   *        {@code VoicePrint}
   */
  ...
  /**
   * Sets the universal model to be used to calculate likelihood ratios Once set, further voice
   * print create / merge operations won't modify this model
   *
  ...
  /**
   * Creates a voice print and stores it along with the user key for later comparison with new
   * samples
   * <p>
  ...
  public VoicePrint createVoicePrint(K userKey, File voiceSampleFile) throws UnsupportedAudioFileException, IOException {
  ...
  private double[] convertFileToDoubleArray(File voiceSampleFile) throws UnsupportedAudioFileException, IOException {
    ...
    if (diff > 5 * Math.ulp(0.0f)) {
      throw new IllegalArgumentException("The sample rate for this file is different than Recognito's " + "defined sample rate : [" + format.getSampleRate() + "]");
    }
    return FileHelper.readAudioInputStream(sample);
  }

  /**
   * Extracts voice features from the given voice sample and merges them with previous voice print
   * extracted for this user key
   * <p>
   * Threading : it is safe to simultaneously add voice samples for a single userKey from multiple
   * threads
   * </p>
   *
   * @param userKey the user key associated with this voice print
   ...
   *
   * @param userKey the user key associated with this voice print
   * @param voiceSampleFile the file containing the voice sample, must have the same sample rate as
   *        defined in constructor
   * @return the updated voice print
   * @throws UnsupportedAudioFileException when the JVM does not support the file format
   * @throws IOException when an I/O exception occurs
   */
  public VoicePrint mergeVoiceSample(K userKey, File voiceSampleFile) throws UnsupportedAudioFileException, IOException {
  ...
  /**
   * Calculates the distance between this voice sample and the voice prints previously extracted and
   * returns the closest matches sorted by distance
   * <p>
   * Usage of a closed set is assumed : the speaker's voice print was extracted before and is known
   * to the system. This means you'll always get MatchResults even if the speaker is absolutely
   * unknown to the system. The MatchResult class provides a likelihood ratio in order to help
  ...
  public Map<K, VoicePrint> getStore() {
    return store;
  }
}
