...
public final class VoicePrint implements Serializable {
  ...
  /**
   * Returns the distance between this voice print and the given one using the calculator. Threading
   * : it is safe to call this method while other threads may merge this voice print instance with
   * another one in the sense that the distance calculation will not happen on half merged voice
   * print. Since this method is read only, it is safe to call it from multiple threads for a single
   * instance
   *
  ...
  /**
   * Merges this voice print features with the given one. Threading : it is safe to call this method
   * while other threads may request the distance of this voice regarding another one in the sense
  ...
  void merge(double[] features) {
    if (this.features.length != features.length) {
      throw new IllegalArgumentException("Features of new VoicePrint is of different size : [" + features.length + "] expected [" + this.features.length + "]");
    }
    w.lock();
    try {
